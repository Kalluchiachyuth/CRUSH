...
function help {
    ...
    echo "================================================================================"
    echo ""
    echo "REFERENCE FILES — choose ONE of the two paths below:"
    echo ""
    echo "  PATH A  -gb  (supported builds: hg19, hg38, mm10, mm9 | res >= 500bp)"
    echo "  -----------------------------------------------------------------------"
    echo "  -gb, --genomebuild BUILD"
    echo "      Automatically downloads chr.sizes, genes.bed, and Bbins.bed"
    echo "      from the JRowleyLab GitHub. You do NOT need -g, -a, or -b."
    echo ""
    echo "      Supported builds : hg19  hg38  mm10  mm9"
    echo "      Resolution limit : res must be >= 500bp"
    echo "                         (Bbins.bed is pre-computed at 500bp; finer"
    echo "                          resolutions require GC recalculation — use PATH B)"
    echo ""
    echo "      If your Hi-C file uses 'chr'-prefixed chromosomes and the reference"
    echo "      files do not (or vice versa), CRUSH auto-converts them to match."
    echo ""
    echo "      Example:"
    echo "        crush -i data.hic -gb hg38 -r 10000 -c 8"
    echo "        crush -i data.hic -gb hg38 -r 10000 -c 8 -o myproject_"
    echo ""
    echo "  PATH B  -g -a -b  (any genome | any resolution)"
    echo "  -----------------------------------------------------------------------"
    echo "  -g, --genomesize FILE   Chromosome sizes file (2 cols: chr_name  size)"
    echo "  -a, --initialA   FILE   Gene annotations BED (initialises A compartments)"
    echo "  -b, --initialB   FILE   Bbins BED  — OR —  genome FASTA"
    echo "                          Use a FASTA when res < 500bp so CRUSH can"
    echo "                          recalculate GC content at your exact resolution."
    echo "                          Use a pre-built Bbins.bed for res >= 500bp."
    echo ""
    echo "      Example (BED):"
    echo "        crush -i data.hic -g hg19.sizes -a genes.bed -b Bbins.bed -r 10000 -c 8"
    echo "      Example (FASTA for sub-500bp):"
    echo "        crush -i data.hic -g hg19.sizes -a genes.bed -b hg19.fa  -r 200  -c 8"
    echo ""
    echo "  MIXING PATHS: you can combine -gb with one or two manual overrides."
    echo "  Any flag you supply explicitly takes priority over the auto-download."
    echo "  Example (override only Bbins):"
    echo "    crush -i data.hic -gb hg38 -b myCustomBbins.bed -r 10000 -c 8"
    echo ""
    echo "================================================================================"
    echo ""
...
function advanced_help {
    ...
    echo "                          Not needed when using -gb for a supported build."
    echo ""
    echo "  -a, --initialA FILE     BED file for initializing A compartments (gene annotations)"
    echo "                          Not needed when using -gb for a supported build."
    echo ""
    echo "  -b, --initialB FILE     BED or FASTA file for initializing B compartments"
    echo "                          BED (Bbins): use for res >= 500bp"
    echo "                          FASTA: use for res < 500bp (GC recalculated)"
    echo "                          Not needed when using -gb for a supported build."
    echo ""
    echo "  -gb, --genomebuild BUILD"
    echo "                          Genome build shortcut: hg19, hg38, mm10, or mm9"
    echo "                          Auto-downloads chr.sizes, genes.bed, Bbins.bed."
    echo "                          Only for res >= 500bp. Overridden by -g, -a, -b"
    echo "                          if those are also supplied."
    echo ""
    ...
    echo "    crush -i data.hic -gb hg38 -r 10000 -c 8 -o myproject_"
    echo ""
    echo "  With manual reference files (BED):"
    echo "    crush -i data.hic -g hg19.sizes -a genes.bed -b Bbins.bed -r 10000 -c 8"
    echo ""
    echo "  With manual reference files (FASTA for sub-500bp):"
    echo "    crush -i data.hic -g hg19.sizes -a genes.bed -b hg19.fa -r 200 -c 8"
    echo ""
    echo "  With multiple threads and custom output:"
    echo "    crush -i data.hic -g hg19.sizes -a genes.bed -b Bbins.bed -r 10000 \\"
...
# ------------------------------------------------------------------------------
# GENOMEBUILD AUTO-DOWNLOAD
# Supported builds: hg19, hg38, mm10, mm9
# Downloads into ${crushdir}/refs/ before the script cd's into ${crushdir}.
# Any flag the user supplies explicitly (-g, -a, -b) overrides the download.
# ------------------------------------------------------------------------------
...
if [ "$genomebuild" != "0" ]; then
    ...
    # Reject sub-500bp: pre-computed Bbins.bed is at 500bp and too coarse.
    # The user must supply a FASTA via -b so GC content can be recalculated.
    if [ -n "$res" ] && [ "$res" -lt 500 ] 2>/dev/null; then
        echo ""
        echo "Note: -gb auto-download is unavailable for resolutions finer than 500bp."
        echo "      Pre-computed Bbins.bed is at 500bp resolution, which is too coarse"
        echo "      for ${res}bp analysis. GC content must be recalculated."
        echo "      Please supply all three reference files manually:"
        echo "        -g  chromosome sizes file"
        echo "        -a  gene annotations BED"
        echo "        -b  genome FASTA (for GC-content-based B-compartment seeds)"
        echo ""
    else
        ...
        if [ "$genomebuild_supported" -eq 0 ]; then
            ...
            JROWLEY_BASE="https://raw.githubusercontent.com/${CRUSH_GITHUB_ORG}/CRUSH/main/Genomes/${genomebuild}"

            _fetch_ref() {
                local url="$1"
                local dest="$2"
                local label="$3"
                # Skip download if a valid file already exists (re-run caching)
                if [ -s "$dest" ]; then
                    echo "Using cached ${label}: ${dest}"
                    return 0
                fi
                ...
                if command -v curl &>/dev/null; then
                    if ! curl -fsSL "$url" -o "$dest"; then
                        rm -f "$dest"
                        echo "Error: curl could not fetch ${label}."
                        echo "  URL tried: ${url}"
                        exit 1
                    fi
                    ...
                    if ! wget -q "$url" -O "$dest"; then
                        rm -f "$dest"
                        echo "Error: wget could not fetch ${label}."
                        echo "  URL tried: ${url}"
                        exit 1
                    fi
                else
                    echo "Error: neither curl nor wget is installed."
                    echo "       Please install one to use -gb, or supply -g, -a, -b manually."
                    exit 1
                fi
                echo "Downloaded ${label} -> ${dest}"
            }

            # Only download files the user has NOT already supplied manually
            if [ -z "$sizefile" ]; then
                _fetch_ref \
                    "${JROWLEY_BASE}/${genomebuild}_chr.sizes" \
                    "${CRUSH_REFCACHE}/${genomebuild}_chr.sizes" \
                    "chr.sizes"
                sizefile="${CRUSH_REFCACHE}/${genomebuild}_chr.sizes"
            fi
...
# At this point -g/-a/-b must be set — either manually or via -gb download.
# ------------------------------------------------------------------------------

if [ "$genomebuild_supported" -eq 0 ] && { [ -z "$sizefile" ] || [ -z "$genesfile" ] || [ -z "$fastafile" ]; }; then
    echo ""
    echo "ERROR: Missing required reference files."
    echo "  Supply either:"
    echo "    -gb BUILD   (hg19 / hg38 / mm10 / mm9, res >= 500bp)"
    echo "  or all three of:"
    echo "    -g  chromosome sizes file"
    echo "    -a  gene annotations BED"
    echo "    -b  Bbins BED (res >= 500bp) or genome FASTA (res < 500bp)"
    echo ""
    help
    exit 1
fi
...
# ------------------------------------------------------------------------------
# CHR PREFIX CHECKER
# Reference files on GitHub use chr1/chr2/chrX (UCSC convention).
# Manual -g/-a/-b files may also use chr1, CHR1, or no prefix (1, 2, X).
# This block detects exactly what prefix the Hi-C file uses, compares it to
# the reference files, and auto-converts in ALL cases — both -gb downloads
# and manually supplied files — so users never have to fix this themselves.
#
# Conversion logic:
#   Step 1: strip any existing chr prefix (chr/Chr/CHR) from col 1
#   Step 2: re-add whatever prefix the Hi-C file actually uses
# This handles all combinations: chr->1, chr->CHR, 1->chr, CHR->chr, etc.
# ------------------------------------------------------------------------------

# Extract the prefix from the reference sizefile (chr / Chr / CHR / "")
...
# _convert_chr: strips any chr prefix then re-adds the Hi-C prefix.
# Works for all cases: chr->1, chr->CHR, 1->chr, CHR->chr, etc.
_convert_chr() {
    local infile="$1"
    local outfile="$2"
...
if [ "$hicchrprefix" != "?" ] && [ "$sizechrprefix" != "$hicchrprefix" ]; then
    ...
    if [ "$genomebuild_supported" -eq 1 ]; then
        # -gb path: auto-convert downloaded files and inform user
        echo ""
        echo "================================================================================"
        echo "  CHROMOSOME NAMING MISMATCH — AUTO-CORRECTED"
        echo "================================================================================"
        echo "  Detected  : Hi-C file uses '${hicchrprefix:-no prefix}' (e.g. '${hicchrprefix:-}1')"
        echo "              Downloaded reference files use '${sizechrprefix:-no prefix}' (e.g. '${sizechrprefix:-}1')"
        echo "  Action    : CRUSH has automatically converted the reference files"
        echo "              to match your Hi-C chromosome naming convention."
        echo "  Continuing: Running with '${hicchrprefix:-no prefix}' chromosome names."
        echo ""
        echo "  NOTE: If the run fails or produces unexpected results, please check"
        echo "        that your Hi-C file chromosome names are consistent throughout"
        echo "        (e.g. not a mix of 'chr1' and '1' in the same file)."
        echo "================================================================================"
        echo ""

        _convert_chr "$sizefile"  "${CRUSH_REFCACHE}/converted_chr.sizes"
        _convert_chr "$genesfile" "${CRUSH_REFCACHE}/converted_genes.bed"
        _convert_chr "$fastafile" "${CRUSH_REFCACHE}/converted_Bbins.bed"

        sizefile="${CRUSH_REFCACHE}/converted_chr.sizes"
        genesfile="${CRUSH_REFCACHE}/converted_genes.bed"
        fastafile="${CRUSH_REFCACHE}/converted_Bbins.bed"

    else
        # Manual -g/-a/-b path: auto-convert and inform user
        echo ""
        echo "================================================================================"
        echo "  CHROMOSOME NAMING MISMATCH — AUTO-CORRECTED"
        echo "================================================================================"
        echo "  Detected  : Hi-C file uses '${hicchrprefix:-no prefix}' (e.g. '${hicchrprefix:-}1')"
        echo "              Your -g/-a/-b files use '${sizechrprefix:-no prefix}' (e.g. '${sizechrprefix:-}1')"
        echo "  Action    : CRUSH has automatically converted your reference files"
        echo "              to match your Hi-C chromosome naming convention."
        echo "  Continuing: Running with '${hicchrprefix:-no prefix}' chromosome names."
        echo ""
        echo "  NOTE: If the run fails or produces unexpected results, please check:"
        echo "    1. That your Hi-C file chromosomes are consistently named throughout"
        echo "       (e.g. not a mix of 'chr1' and '1' in the same file)"
        echo "    2. That your -g, -a, and -b files all use the same naming convention"
        echo "    3. To avoid this message in future runs, update your -g/-a/-b files"
        echo "       to use '${hicchrprefix:-no prefix}' chromosome names before running CRUSH"
        echo "================================================================================"
        echo ""

        CRUSH_CONVERTDIR="$(pwd)/${crushdir}/converted_refs"
        mkdir -p "${CRUSH_CONVERTDIR}"

        _convert_chr "$sizefile"  "${CRUSH_CONVERTDIR}/converted_chr.sizes"
        _convert_chr "$genesfile" "${CRUSH_CONVERTDIR}/converted_genes.bed"
        _convert_chr "$fastafile" "${CRUSH_CONVERTDIR}/converted_Bbins.bed"

        sizefile="${CRUSH_CONVERTDIR}/converted_chr.sizes"
        genesfile="${CRUSH_CONVERTDIR}/converted_genes.bed"
        fastafile="${CRUSH_CONVERTDIR}/converted_Bbins.bed"
    fi
fi
